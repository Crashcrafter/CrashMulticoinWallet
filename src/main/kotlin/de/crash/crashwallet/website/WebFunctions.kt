package de.crash.crashwallet.website

import de.crash.crashwallet.AccountTable
...
//With Settings etc
data class FullUserData(val name: String, val role : String, val email: String, val loggedIn : Boolean)
...
fun getFullUserData(userInfo: UserInfo?) : FullUserData {
	if(userInfo == null) return FullUserData("", "", "",false)
	var name = ""
	var role = ""
	var email = ""
	val userId = userInfo.id
	...
	transaction {
		...
		if(exists){
			AccountTable.select { AccountTable.id eq userId }.firstOrNull()?.let {
				name = it[AccountTable.username].toString()
				role = it[AccountTable.role].toString()
				email = it[AccountTable.email].toString()
			}
		}
	}
	if(!exists){
		return FullUserData("", "","", false)
	}
	val loggedin = userInfo.loggedin
	return FullUserData(name, role, email, loggedin)
}
